...
/// </summary>
/// <remarks>
/// One place, so that five factories refuse the same things in the same words. A name is checked for
...
internal static class OrleansBindingNames
{
    ...
    /// </exception>
    /// <remarks>
    /// The well-formedness scanner is the core package's, the same one the JSON string writer uses, and
    /// sharing it is the point: one implementation of "is this text" is what keeps the refusal here and the
    /// writing there from ever disagreeing about which names are whole.
    /// </remarks>
    internal static void Require(string name)
    {
        ...
        {
            throw new ArgumentException(
                "The name of a named Orleans binding carries an unpaired surrogate, so it is not well-formed text and has no exact form on the wire. A document written from it would substitute the replacement character for that character, which would let two different names address one binding and would store a name that is not the one registered here.",
                nameof(name));
        }
    }
