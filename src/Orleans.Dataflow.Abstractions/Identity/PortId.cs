...
/// <summary>
/// Identity of a single input or output port on a node.
/// </summary>
/// <remarks>
/// <para>
/// Edges in a graph document connect a node/port pair to another node/port pair, so a port identifier
/// is only meaningful relative to the node that declares it.
/// </para>
/// <para>
...
public readonly record struct PortId
{
    ...
    private PortId(string value) => _value = value;
    ...
    /// </exception>
    public string Value =>
        _value ?? throw new InvalidOperationException(IdentifierGrammar.DescribeDefaultAccess(nameof(PortId)));
    ...
    /// <summary>
    /// Creates a <see cref="PortId"/> from its text form.
    /// </summary>
    ...
    /// </exception>
    public static PortId Create(string value)
    {
        IdentifierGrammar.EnsureSegment(value, nameof(PortId), nameof(value));
        return new PortId(value);
    }

    /// <summary>
    /// Attempts to create a <see cref="PortId"/> from its text form.
    /// </summary>
    ...
    public static bool TryCreate(string? value, out PortId identifier)
    {
        if (value is not null && IdentifierGrammar.IsSegment(value))
        {
            identifier = new PortId(value);
            return true;
        }
    ...
    /// </summary>
    /// <returns>
    /// The identifier text, or <c>"(default PortId)"</c> when <see cref="IsDefault"/> is
    /// <see langword="true"/>.
    /// </returns>
    /// <remarks>
    ...
    /// </remarks>
    public override string ToString() => _value ?? "(default PortId)";
}
