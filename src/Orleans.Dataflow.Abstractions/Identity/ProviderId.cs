...
/// <summary>
/// Stable identity of a stage catalog provider.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public readonly record struct ProviderId : IComparable<ProviderId>, IComparable
{
    private readonly string? _value;

    private ProviderId(string value) => _value = value;
    ...
    /// </exception>
    public string Value =>
        _value ?? throw new InvalidOperationException(IdentifierGrammar.DescribeDefaultAccess(nameof(ProviderId)));
    ...
    /// <summary>
    /// Creates a <see cref="ProviderId"/> from its text form.
    /// </summary>
    ...
    {
        IdentifierGrammar.EnsureSegment(value, nameof(ProviderId), nameof(value));
        return new ProviderId(value);
    }

    /// <summary>
    /// Attempts to create a <see cref="ProviderId"/> from its text form.
    /// </summary>
    ...
    public static bool TryCreate(string? value, out ProviderId identifier)
    {
        if (value is not null && IdentifierGrammar.IsSegment(value))
        {
            identifier = new ProviderId(value);
            return true;
        }
    ...
    /// </remarks>
    public int CompareTo(ProviderId other) => string.CompareOrdinal(_value, other._value);
    ...
    /// </returns>
    /// <exception cref="ArgumentException"><paramref name="obj"/> is not a <see cref="ProviderId"/>.</exception>
    /// <remarks>
    ...
    /// <see cref="CompareTo(ProviderId)"/> instead and box nothing.
    /// </remarks>
    int IComparable.CompareTo(object? obj) => obj switch
    {
        null => 1,
        ProviderId other => CompareTo(other),
        _ => throw new ArgumentException($"The argument must be a {nameof(ProviderId)}.", nameof(obj)),
    };
    ...
    /// </summary>
    /// <returns>
    /// The identifier text, or <c>"(default ProviderId)"</c> when <see cref="IsDefault"/> is
    /// <see langword="true"/>.
    /// </returns>
    /// <remarks>
    ...
    /// </remarks>
    public override string ToString() => _value ?? "(default ProviderId)";
}
