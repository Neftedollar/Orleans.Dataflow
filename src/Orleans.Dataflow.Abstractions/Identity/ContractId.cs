...
/// <summary>
/// Stable identity of a versioned data contract.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public readonly record struct ContractId : IComparable<ContractId>, IComparable
{
    private readonly string? _value;

    private ContractId(string value) => _value = value;
    ...
    /// </exception>
    public string Value =>
        _value ?? throw new InvalidOperationException(IdentifierGrammar.DescribeDefaultAccess(nameof(ContractId)));
    ...
    /// <summary>
    /// Creates a <see cref="ContractId"/> from its text form.
    /// </summary>
    ...
    /// </exception>
    public static ContractId Create(string value)
    {
        IdentifierGrammar.EnsureSegment(value, nameof(ContractId), nameof(value));
        return new ContractId(value);
    }

    /// <summary>
    /// Attempts to create a <see cref="ContractId"/> from its text form.
    /// </summary>
    ...
    public static bool TryCreate(string? value, out ContractId identifier)
    {
        if (value is not null && IdentifierGrammar.IsSegment(value))
        {
            identifier = new ContractId(value);
            return true;
        }
    ...
    /// otherwise <see langword="false"/>.
    /// </returns>
    public static bool operator <(ContractId left, ContractId right) => left.CompareTo(right) < 0;
    ...
    /// </returns>
    /// <exception cref="ArgumentException"><paramref name="obj"/> is not a <see cref="ContractId"/>.</exception>
    /// <remarks>
    ...
    /// <see cref="CompareTo(ContractId)"/> instead and box nothing.
    /// </remarks>
    int IComparable.CompareTo(object? obj) => obj switch
    {
        null => 1,
        ContractId other => CompareTo(other),
        _ => throw new ArgumentException($"The argument must be a {nameof(ContractId)}.", nameof(obj)),
    };
    ...
    /// </summary>
    /// <returns>
    /// The identifier text, or <c>"(default ContractId)"</c> when <see cref="IsDefault"/> is
    /// <see langword="true"/>.
    /// </returns>
    /// <remarks>
    ...
    /// </remarks>
    public override string ToString() => _value ?? "(default ContractId)";
}
